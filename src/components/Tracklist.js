...
function Tracklist( {tracks, token} ) {
    ...
    useEffect(() => {
      ...
      useEffect(()=> {
        if (DisplayBPM && DisplayLikes && DisplayPreviews){
          ...
          console.log('tracks in the end', tracks)
          setTracks(tracks)
        }
 ...
 .catch(error => console.error('(1) Inside error:', error))
  ...
  if (DisplayPreviews && DisplayBPM) {
    ...
    <ul >
        ...
        <Track track={track} token={token} audiofeat={track.audio_features} trackdetails={track.track_details} like={Likes[index]}/>
        </li>
        ))}
